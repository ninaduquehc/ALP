...
def cat_dog(s):
    ...
    for a in range(len(s)-2):
        if s[a]=='c' and s[a+1]=='a' and s[a+2]=='t':
            gato=gato+1
        if s[a]=='d' and s[a+1]=='o' and s[a+2]=='g':
            cachorro=cachorro+1
    return gato==cachorro
...
def count_code(s):
    ...
    for a in range(len(s)-3):
        if s[a]=='c' and s[a+1]=='o' and s[a+3]=='e':
            soma=soma+1
    return soma
