#LISTA 1
#Marina Duque de Holanda Cavalcanti - DSM(1S26)

#1 Faça um programa que peça dois números inteiros e imprima a soma desses dois número
x=int(input('valor1: '))
y=int(input('valor2: '))
print (x+y)

#2 Escreva um programa que leia um valor em metros e o exiba convertido em milímetro
metro=float(input('valor: '))
print(f'milímetros:{1000*metro}')

#3 Escreva um programa que leia a quantidade de dias, horas, minutos e segundos do usuário. Calcule o total em segundos
d=int(input('dias: '))
h=int(input('horas: '))
m=int(input('minutos: '))
s=int(input('segundos: '))
total=d*24*60*60 + h*60*60 + m*60 +s
print (total)

#4 Faça um programa que calcule o aumento de um salário. Ele deve solicitar o valor do salário e a porcentagem do aumento. Exiba o valor do aumento e do novo salário.
s=float(input('salário: '))
p=float(input('porcentagem de aumento em decimal: '))
a=s*p
print (f'aumento: R$ {a:.2f}')
print (f'novo salário: R$ {a+s:.2f}')

#5 Solicite o preço de uma mercadoria e o percentual de desconto. Exiba o valor do desconto e o preço a pagar.
p=float(input('preço: '))
d=float(input('desconto (em decimal): '))
v=d*p
print (f'valor do desconto: {v:.2f}')
print (f'preço a pagar: {p-v:.2f}')

#6 Calcule o tempo de uma viagem de carro. Pergunte a distância a percorrer e a velocidade média esperada para a viagem.
d=float(input('distância em km: '))
v=float(input('velocidade média em km/h: '))
t=d/v
if t>1:
    print (f'hora(s): {t:.1f}')
if t<1:
    print (f'minuto(s): {t*60:.1f}')

#7 Converta uma temperatura digitada em Celsius para Fahrenheit. F = 9*C/5 + 32 8)
#Faça agora o contrário, de Fahrenheit para Celsius.
c=float(input('temperatura(celcius): '))
f=9*c/5+32.8
print (f'Fahrenheit:{f}')

f=float(input('temperatura(fahrenheit): '))
c=5/9*(f-32.8)
print (f'celcius:{c}')

#9 Escreva um programa que pergunte a quantidade de km percorridos por um carro alugado pelo usuário, assim como a quantidade de dias pelos quais o carro foi alugado. Calcule o preço a pagar, sabendo que o carro custa R$ 60,00 por dia e R$ 0,15 por km rodado.
k=float(input('kilômetros: '))
d=int(input('dias: '))
p=60*d+0.15*k
print (f'valor: R$ {p:.2f}')

#10 Escreva um programa para calcular a redução do tempo de vida de um fumante. Pergunte a quantidade de cigarros fumados por dia e quantos anos ele já fumou. Considere que um fumante perde 10 minutos de vida a cada cigarro, calcule quantos dias de vida um fumante perderá. Exiba o total de dias.
c=int(input('quantidade de cigarros por dia: '))
a=int(input('quantos anos de fumo: '))
x=c*365*a*10/1440
print (f'dias perdidos: {x:.2f}')

#11 Sabendo que str( ) converte valores numéricos para string, calcule quantos dígitos há em 2 elevado a dez mil
n=2**10000
q=len(str(n))
print(f'{q} dígitos')
